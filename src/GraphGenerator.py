...
class GraphGenerator:
    ...
    def generate_graph(self) -> dict[str, Any]:
        ...
        measures: dict[str, int] = GraphGenerator.get_area(self.hubs)
        width: int = (measures["x_max"] - measures["x_min"]) + 1
        height: int = (measures["y_max"] - measures["y_min"]) + 1

        graph: list[list[Zone | int]] = []
        graph = GraphGenerator.fill_graph(graph, height, width)
        y_offset, x_offset = GraphGenerator.get_offset(measures)
